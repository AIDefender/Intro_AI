...
def save_model(ep,agents):
    ...
    if not os.path.exists("../saved_model/CNN_DQN"):
        os.mkdir('../saved_model/CNN_DQN')
    agents[0].save(checkpoint_root='../saved_model/CNN_DQN', checkpoint_name='{}'.format(ep+1))
...
def restore_model(agents,path=None):
    ...
    if path:
        ...
        idex = get_max_idx("../saved_model/CNN_DQN")
        path = os.path.join("../saved_model/CNN_DQN",str(idex))
        agents[0].restore(path)
