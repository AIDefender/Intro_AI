...
public class MTDDecider implements Decider {
	...
	private int MTDF(State root, int firstGuess, int depth)
		...
		while (lowerbound < upperbound) {
			...
			//! In MTD(f), AlphaBeta fails high or low, returning a lower bound or an 
			//! upper bound on the minimax value, respectively. Zero-window calls cause
			//! more cutoffs, but return less information - only a bound on the minimax
			//! value. To find the minimax value, MTD(f) calls AlphaBeta a number of
			//! times, converging towards it and eventually finding the exact value.
